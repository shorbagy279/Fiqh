...
const QuizOptionsModal = ({ onClose, onStart, categories, type = 'random' }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end justify-center z-50 animate-slide-in">
      <div className="bg-white rounded-t-3xl w-full max-w-md max-h-[85vh] overflow-auto shadow-2xl">
        {/* Header */}
        <div className="sticky top-0 bg-gradient-to-r from-green-600 to-green-700 text-white p-6 rounded-t-3xl shadow-lg z-10">
              ...
              >
                <div
                  className={`absolute w-5 h-5 bg-white rounded-full top-1 transition-all shadow-md ${
                    showCustomMode ? 'right-1' : 'right-8'
                  }`}
                ></div>
              </button>
            </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        {isSelected && (
                          <div className="bg-white text-green-600 w-8 h-8 rounded-full flex items-center justify-center font-bold shadow-md">
                            ✓
                          </div>
                        )}
                      </div>
              ...
              {selectedCategories.length > 0 && (
                <div className="mt-3 bg-blue-50 border-2 border-blue-200 rounded-xl p-3">
                  <p className="text-sm text-blue-800 text-center">
                    ستحصل على أسئلة من {selectedCategories.length} {selectedCategories.length === 1 ? 'قسم' : 'قسم'}
                  </p>
                </div>
              )}
            </div>
